...
int compression(const std::string &input_filename, const std::string &output_filename) {
    ...
    huffman::encoder<uint8_t> encoder(frequencies);
    ...
    for (const auto &[symbol, length, code] : encoder.table()) {
        writer.write(symbol, 8).write(length, 5).write(code, length);
    }
...
int decompression(const std::string &input_filename, const std::string &output_filename) {
    ...
    huffman::decoder<uint8_t> decoder(table);
