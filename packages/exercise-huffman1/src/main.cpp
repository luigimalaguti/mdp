...
namespace huffman_encode {
    ...
    struct compare_nodes {
        bool operator()(const encode_node *lhs, const encode_node *rhs) const {
            return lhs->frequency_ > rhs->frequency_;
        }
    };

    void generate_codes(std::unordered_map<uint8_t, std::pair<uint8_t, uint32_t>> &table, encode_node *node,
                        uint8_t length = 0, uint32_t code = 0) {
        if (node->is_leaf()) {
            table[node->symbol_] = std::make_pair(length, code);
        } else {
            generate_codes(table, node->left_, length + 1, (code << 1) | 0x0);
            generate_codes(table, node->right_, length + 1, (code << 1) | 0x1);
        }
    }
    ...
    int compression(const std::string &input_filename, const std::string &output_filename) {
        ...
        while (nodes.size() > 1) {
            ...
            auto it = std::upper_bound(nodes.begin(), nodes.end(), parent, compare_nodes());
        ...
        generate_codes(codes_table, root);
        ...
        for (const auto &[symbol, pair_code] : codes_table) {
            const auto &[length, code] = pair_code;
...
namespace huffman_decode {
    ...
    int decompression(const std::string &input_filename, const std::string &output_filename) {
        ...
        for (size_t index = 0; index < table_entries; index++) {
            uint64_t raw_symbol = 0, raw_length = 0, raw_code = 0;
            reader.read(raw_symbol, 8);
            reader.read(raw_length, 5);
            reader.read(raw_code, raw_length);
            codes_table.push_back(
                {static_cast<uint8_t>(raw_symbol), static_cast<uint8_t>(raw_length), static_cast<uint32_t>(raw_code)});
        }
        std::sort(codes_table.begin(), codes_table.end());
